...
local function categorize_chapters()
	...
	for i, chapter in ipairs(chapter_list) do
		...
		msg.info(
			string.format(
				"Chapter %d: '%s' | Start: %.2fs | Length: %.2fs | %s",
				i,
	...
	for i = 1, #chapters - 1 do
		...
		if not current.skippable and next.skippable then
			...
			msg.info(
				string.format(
					"Transition %d->%d: %.2fs to %.2fs (boundary: %.2fs)",
					i,
					i + 1,
					range_start,
					range_end,
...
local function countdown_tick()
	...
	msg.info(
		string.format("[COUNTDOWN] Chapter %d countdown: %d", active_prompt.chapter_index, active_prompt.countdown)
	)

	if active_prompt.countdown <= 0 then
		-- Time to skip
		msg.info(string.format("[AUTO-SKIP] Countdown reached 0, skipping chapter %d", active_prompt.chapter_index))
		skip_now()
		return
	end
...
local function trigger_prompt(chapter_index, prompt_type, countdown_duration)
	...
	msg.info(
		string.format(
			"[TRIGGER] Starting %s prompt for chapter %d ('%s') with %ds countdown",
			prompt_type,
			chapter_index,
			get_chapter_name(chapter_index),
			countdown_duration
		)
	)
...
local function monitor_position()
	...
	if range then
		...
		if not active_prompt or active_prompt.chapter_index ~= range.target_chapter then
			msg.info(
				string.format(
					"[MONITOR] Entered transition range at %.2fs, targeting chapter %d",
					current_time,
					range.target_chapter
				)
			)
		...
		if active_prompt and active_prompt.type == "case2" and last_range_check then
			msg.info(string.format("[MONITOR] Left transition range at %.2fs, cancelling case2 prompt", current_time))
...
mp.add_key_binding("Shift+ç", "add-time", add_time_to_countdown)
